...
def find_best(parquets_meta, df_abc, mode, sku_or_class, fill_rate_target=95.0):
    ...
    """
    # Convert to list of dicts if passed as tuple of tuples
    if parquets_meta and isinstance(parquets_meta[0], (list, tuple)):
        parquets_meta = [{"filepath": p[0], "policy": p[1], "ns": p[2], "k": p[3]} for p in parquets_meta]
    rows = []
...
if mode == "Overview":
    ...
    if df_abc is not None and "sku" in df_all.columns:
        ...
        for cls in classes_list:
            ...
            res_cls = find_best(tuple((m["filepath"], m["policy"], m["ns"], m["k"]) for m in parquets_meta), df_abc, "CLASS", cls, fill_rate_target=95.0)
            if res_cls:
...
with st.spinner("Evaluating all combinations..."):
    res = find_best(tuple((m["filepath"], m["policy"], m["ns"], m["k"]) for m in parquets_meta), df_abc, bc_mode, sku_or_class, fill_rate_target)
