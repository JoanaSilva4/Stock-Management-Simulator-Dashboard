...
COLS_NEEDED = [
    "sku", "date", "soh_final", "demand", "forecast", "stockout",
    "encomendas_pedidas", "encomendas_colocadas", "num_encomendas",
    "SS", "SS_s", "ss", "safety_stock",
    "custo_stock_diario", "custo_stock", "stock_cost",
    "custo_transporte", "transport_cost", "custo_transporte_diario",
...
def load_parquet(path):
    ...
    cols_to_load = [c for c in COLS_NEEDED if c in schema_cols]
    df = pd.read_parquet(path, columns=cols_to_load if cols_to_load else None)
    if "date" in df.columns:
        df["date"] = pd.to_datetime(df["date"], dayfirst=True, errors="coerce")
    # Converter colunas numéricas para tipos mais leves
