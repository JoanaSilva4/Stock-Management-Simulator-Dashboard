...
def load_moq_params(folder):
    ...
    ns_m = re.search(r"simulator[_\s]?(\d+)", base, re.IGNORECASE)
    k_m  = re.search(r"k[=_](\d+(?:\.\d+)?)", base, re.IGNORECASE)
    ns   = ns_m.group(1) if ns_m else "N/A"
    k    = k_m.group(1)  if k_m  else "N/A"
    policy = "N/A"
    if ns_m and k_m:
        policy = base[ns_m.end():k_m.start()].strip("_- ").strip()
    elif ns_m:
        policy = base[ns_m.end():].strip("_- ").strip()
    if not policy:
        policy = "N/A"
    return {"ns": ns, "policy": policy, "k": k, "filepath": filepath}
...
with st.sidebar:
    ...
    folder = st.text_input(
        "fi", value=default_folder, label_visibility="collapsed",
...
sub = "Global Overview — all files averaged" if mode == "Overview" else \
      (f"SKU {sku_sel}" if mode == "By SKU" else f"Class {class_sel}")

col_h, col_kpi = st.columns([3,1])
with col_h:
    st.markdown(f"""
    <div class="db-title">📦 Stock Simulator <span>Dashboard</span></div>
    <div class="db-sub">{sub}{f' &nbsp;·&nbsp; {pol_disp} &nbsp;·&nbsp; NS {ns_sel}% &nbsp;·&nbsp; k={k_sel}' if mode != 'Overview' else ''}</div>
    """, unsafe_allow_html=True)
with col_kpi:
    st.markdown(f"""
...
if mode == "Overview":
    ...
    for col, val, lbl, cls in [
        ...
        (c3, f"{n_days_sim:,}",  "Simulated Days"," days",  ""),
    ...
    if df_abc is not None and "sku" in df_all.columns:
        st.markdown("<div class='sec-hdr'>◆ Performance by ABC/XYZ Class</div>",
                    unsafe_allow_html=True)
        df_with_class = df_all.merge(df_abc, on="sku", how="left")
        if "Classe_ABC_XYZ" in df_with_class.columns:
            df_cls_agg = df_with_class.groupby("Classe_ABC_XYZ").agg(
                n_skus=("sku","nunique"),
                fill_rate=("fill_rate","mean"),
                csl=("csl","mean"),
                stockout_days=("stockout_days","mean"),
                ...
                cost_total=("cost_total","mean"),
            ).reset_index().sort_values("Classe_ABC_XYZ")

            col_cls_bar, col_cls_tbl = st.columns([1,1])
            with col_cls_bar:
                fig_cls = go.Figure()
                fig_cls.add_trace(go.Bar(
                    x=df_cls_agg["Classe_ABC_XYZ"], y=df_cls_agg["fill_rate"],
                    marker_color=PETROL, name="Avg Fill Rate β (%)",
                ))
                fig_cls.update_layout(**PLOT, height=260, margin=dict(l=10,r=10,t=30,b=10),
                                      title="Avg Fill Rate β by ABC/XYZ Class",
                                      yaxis=dict(gridcolor=GREY_BDR, title="%"),
                                      xaxis=dict(gridcolor=GREY_BDR))
                st.plotly_chart(fig_cls, use_container_width=True)

            with col_cls_tbl:
                df_cls_show = df_cls_agg.rename(columns={
                    "Classe_ABC_XYZ":"Class","n_skus":"Number of SKUs",
                    "fill_rate":"Fill Rate β (%)","csl":"CSL α (%)",
                    "stockout_days":"Stockout Days","cobertura":"Coverage (days)",
                    "cost_total":"Avg Cost (€)"
                })
                fmt_cls = {k:v for k,v in {
                    "Fill Rate β (%)":"{:.2f}","CSL α (%)":"{:.2f}",
                    "Stockout Days":"{:.1f}","Coverage (days)":"{:.0f}",
                    "Avg Cost (€)":"{:,.2f}"
                }.items() if k in df_cls_show.columns}
                grad_cls = [c for c in ["Fill Rate β (%)","CSL α (%)"] if c in df_cls_show.columns]
                styled_cls = df_cls_show.style.format(fmt_cls, na_rep="—")
                if grad_cls:
                    styled_cls = styled_cls.background_gradient(subset=grad_cls, cmap="RdYlGn", vmin=80, vmax=100)
                st.dataframe(styled_cls, use_container_width=True, height=260, hide_index=True)
    ...
    if not df_sku_avg.empty:
        ...
        with col_hist:
            ...
            fig_h.update_layout(**PLOT, height=270, margin=dict(l=10,r=10,t=30,b=10),
                                ...
                                yaxis=dict(title="# SKUs", gridcolor=GREY_BDR))
...
if res:
    ...
    with col_b:
        ...
        <div class="bestbox">
          ...
          <div class="best-combo">
            {policy_label(best['policy'])} · NS {best['ns']}% · k={best['k']}
            {badge_html}
          </div>
    ...
    with col_t:
        ...
        df_show = df_show[["Policy","NS (%)","k","Fill Rate β (%)","Total Cost (€)","Meets Target"]] \
...
if "date" in df_filtered.columns and "soh_final" in df_filtered.columns:
    if mode == "By SKU":
        ...
        ss_c = next((c for c in ["SS_s","ss","safety_stock","SS"] if c in df_plot.columns), None)
        if ss_c:
            fig.add_trace(go.Scatter(
                x=df_plot["date"], y=df_plot[ss_c],
                mode="lines", name="Safety Stock",
        ...
        if "stockout" in df_plot.columns:
            ...
            if not so.empty:
                fig.add_trace(go.Scatter(
                    x=so["date"], y=so["soh_final"], mode="markers", name="Stockout",
...
if cs_c and ct_c and "date" in df_filtered.columns:
    ...
    with col_pie:
        fig_p = go.Figure(go.Pie(
            ...
            hole=0.55, marker=dict(colors=[PETROL, ORANGE]),
        ))
